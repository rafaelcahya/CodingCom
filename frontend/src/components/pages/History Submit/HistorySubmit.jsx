...
export default function HistorySubmit(props) {
    ...
    const OpenSetButtonPopup1 = (id) => {
        // useEffect(() => {
        //     axios.get("http://localhost:3001/submit/submitListByIdDetail/" + urlid).then((response) => {
        //         setValueList(response.data)
        //         console.log(response.data)
        //     })
        //     // eslint-disable-next-line react-hooks/exhaustive-deps
        // }, []);
        axios.get("http://localhost:3001/submit/submitListByIdDetail/" + id).then((response) => {
            setValueList(response.data)
        })
                                        ...
                                        <div className="history-project-card flex flex-col gap-3 mt-5 p-5 hover:shadow-lg transform hover:scale-105 duration-200 rounded-lg w-full border-b">
                                            ...
                                            <div className="flex flex-col md:justify-between gap-5">
                                                ...
                                                <div className="flex items-center justify-end gap-5 text-sm">
                                                    <p className="text-blue-500 font-medium cursor-pointer" onClick={() => { OpenSetButtonPopup1(item.id) }}>More project details</p>
                                                    {
                                                ...
                                                return <div>
                                                    <div className="flex justify-between mt-14">
                                                        ...
                                                        <div className="flex flex-col gap-5 text-sm">
                                                            {
                                                                item.score >= 95 ? (<Link to={"/CertificateAngular/" + v.project_id}>
                                                            ...
                                                            <div>
                                                                {
                                                                    item.score < 95 ? <Link to={"/submit-solution/" + v.project_id}><p className="text-white text-center bg-blue-1 px-4 py-2 rounded-lg">Submit solution</p>
                                                                    </Link> : <p className="text-center px-4 py-2 rounded-lg">Submit solution</p>
                                                                }
                                                            </div>
                                                        </div>
...
function Menu({ menuItem }) {
    ...
    const OpenSetButtonPopup2 = (id) => {
        axios.get("http://localhost:3001/submit/submitListByIdDetail/" + id).then((response) => {
            setValueList2(response.data)
        })
                    ...
                    return <div>
                        <div className="history-project-card flex flex-col gap-3 mt-5 p-5 hover:shadow-lg transform hover:scale-105 duration-200 rounded-lg w-full border-b">
                            ...
                            <div className="flex flex-col md:justify-between gap-5">
                                ...
                                <div className="flex items-center justify-end gap-5 text-sm">
                                    <p className="text-blue-500 font-medium cursor-pointer" onClick={() => { OpenSetButtonPopup2(item.id) }}>More project details</p>
                                    {
                                        item.score >= 95 ? (<Link to={"/CertificateAngular/" + item.project_id}>
